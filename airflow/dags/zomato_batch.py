...
DBT_PROFILE= "/opt/airflow/dbt/zomato"
...
with DAG(
    ...
    dbt_build_core = BashOperator(
        ...
        bash_command=f"{DBT} build --exclude tag:ai --project-dir {DBT_PROJECT} --profiles-dir {DBT_PROFILE}",
    )
    ...
    dbt_build_ai = BashOperator(
        ...
        bash_command=f"{DBT} build --select tag:ai --project-dir {DBT_PROJECT} --profiles-dir {DBT_PROFILE}"
    )
