...
from groq import Groq  # 1. Swapped OpenAI for Groq
import numpy as np
import pandas as pd
...
# 2. Updated to a Groq-supported model (e.g., Llama 3.3 70B)
...
- Always prefix tables with MARTS schema (e.g., MARTS.MART_RESTAURANT_PERFORMANCE or MARTS.FCT_ORDERS).
...
def generate_sql(question):
    ...
    # Sanitize prefix if LLM hallucinates full paths
...
st.caption(f"Ask in English, {MODEL} (via Groq) writes the SQL, Snowflake runs it")
...
if question:
    ...
    if not is_safe(sql):
        st.error(
            "The generated SQL is not safe to run. Please modify your question."
        )
    else:
        try:
            df = run_query(sql)
            ...
            # Auto-charting logic
            if len(df.columns) == 2 and pd.api.types.is_numeric_dtype(
                df.iloc[:, 1]
            ):
