...
# Initialize local embedding model (runs 100% free locally on CPU)
@st.cache_resource
...
def embed(texts):
    # Generates dense vectors locally using FastEmbed
...
st.caption(f"Searching {NEW_REVIEWS} reviews, answering with Groq ({CHAT_MODEL})")
...
def ask_llm(question, top_reviews):
    ...
    # Groq SDK uses exact same syntax as OpenAI
